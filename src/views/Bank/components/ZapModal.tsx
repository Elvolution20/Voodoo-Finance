...
import { VOODOO_TICKER, VSHARE_TICKER, BTT_TICKER } from '../../../utils/constants';
...
const ZapModal: React.FC<ZapProps> = ({ onConfirm, onDismiss, tokenName = '', decimals = 18 }) => {
  ...
  const bttBalance = (Number(balance) / 1e18).toFixed(4).toString();
  const voodooBalance = useTokenBalance(voodooFinance.VOODOO);
  const vshareBalance = useTokenBalance(voodooFinance.VSHARE);
  const [val, setVal] = useState('');
  const [zappingToken, setZappingToken] = useState(BTT_TICKER);
  const [zappingTokenBalance, setZappingTokenBalance] = useState(bttBalance);
  const [estimate, setEstimate] = useState({ token0: '0', token1: '0' }); // token0 will always be BTT in this case
  const [approveZapperStatus, approveZapper] = useApproveZapper(zappingToken);
  const voodooBttLpStats = useLpStats('VOODOO-BTT-LP');
  const vShareBttLpStats = useLpStats('VSHARE-BTT-LP');
  const voodooLPStats = useMemo(() => (voodooBttLpStats ? voodooBttLpStats : null), [voodooBttLpStats]);
  const vshareLPStats = useMemo(() => (vShareBttLpStats ? vShareBttLpStats : null), [vShareBttLpStats]);
  const bttAmountPerLP = tokenName.startsWith(VOODOO_TICKER) ? voodooLPStats?.bttAmount : vshareLPStats?.bttAmount;
  /**
  ...
  const handleChangeAsset = (event: any) => {
    ...
    setZappingTokenBalance(bttBalance);
  ...
  return (
    <Modal>
      ...
      <Select
        ...
        value={zappingToken}
      >
        <StyledMenuItem value={BTT_TICKER}>BTT</StyledMenuItem>
      ...
      <StyledDescriptionText>
        {' '}
        {tokenName}: {Number(estimate.token0) / Number(bttAmountPerLP)}
      </StyledDescriptionText>
      <StyledDescriptionText>
        {' '}
        ({Number(estimate.token0)} {BTT_TICKER} / {Number(estimate.token1)}{' '}
