...
import { BTT_TICKER, VOODOO_TICKER, VSHARE_TICKER, ZAPPER_ROUTER_ADDR } from '../utils/constants';
...
function useApproveZapper(zappingToken: string): [ApprovalState, () => Promise<void>] {
  ...
  let token: ERC20;
  if (zappingToken === BTT_TICKER) token = voodooFinance.BTT;
  ...
  const approvalState: ApprovalState = useMemo(() => {
    ...
    if (token === voodooFinance.BTT) return ApprovalState.APPROVED;
