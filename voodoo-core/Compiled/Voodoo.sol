...
contract Voodoo is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 VOODOO-WBTT LP -> VOODOO pool
    uint256 public constant INITIAL_VOODOO_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the VOODOO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("VOODOO", "VOODOO") {
        // Mints 1 VOODOO to contract creator for initial pool setup
    ...
    function _getVoodooPrice() internal view returns (uint256 _voodooPrice) {
        try IOracle(voodooOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Voodoo: failed to fetch VOODOO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _voodooPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_voodooPrice >= taxTiersTwaps[tierId]) {
    ...
    function setVoodooOracle(address _voodooOracle) public onlyOperatorOrTaxOffice {
        require(_voodooOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints VOODOO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of VOODOO to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentVoodooPrice = _getVoodooPrice();
            currentTaxRate = _updateTaxRate(currentVoodooPrice);
            if (currentVoodooPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_voodooPool != address(0), "!_voodooPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_voodooPool, INITIAL_VOODOO_POOL_DISTRIBUTION);
